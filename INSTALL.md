...
首次运行 `npm run dev` 时，Strapi 会在控制台引导你创建第一个管理员账号：
...
**注意**：请妥善保管此账号，它是系统最高权限账号。

#### 2. 自动创建根渠道

系统启动后约 5 秒，`zhao-channel` 插件会自动执行以下初始化操作：
...
[zhao-channel] admin 角色已关联到根渠道 (Role ID: 1)
[zhao-channel] admin 用户渠道权限已授予 (User ID: 1)
```

**幂等性保障**：如果根渠道已存在，系统会跳过初始化，不会重复创建。

#### 3. 验证初始化结果

登录后台管理系统 `http://localhost:1337/admin` 后：
- 管理员账号拥有全部渠道权限
- 可在渠道管理中看到"平台根渠道"
- 可创建子渠道、租户等
...
如果自动初始化失败，可通过以下方式手动操作：

### 使用初始化脚本（推荐）

项目提供了专门的初始化脚本，可创建根渠道和渠道管理员用户：
...
**手动创建根渠道**：
```bash
# 使用 Strapi CLI 或通过 API 创建
```

**手动关联用户到渠道**：
在后台管理系统中，将 admin 用户添加到渠道成员列表，角色选择"owner"。

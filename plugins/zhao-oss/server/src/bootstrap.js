...
const bootstrap = async ({ strapi }) => {
    ...
    if (!isTest)
        logger.info("[zhao-oss] Initializing OSS backup plugin...");
    const pluginConfig = strapi.config.get("plugin::zhao-oss");
    if (!(pluginConfig === null || pluginConfig === void 0 ? void 0 : pluginConfig.enabled)) {
        if (!isTest)
            logger.info("[zhao-oss] Plugin is disabled, skipping initialization");
        return;
    }
    ...
    try {
        ...
        for (const f of staleFiles) {
            ...
            if ((meta === null || meta === void 0 ? void 0 : meta.localUrl) && !meta.localUrl.startsWith("http") && !meta.localUrl.startsWith("/static")) {
    ...
    try {
        ...
        for (const def of DEFAULT_FOLDERS) {
            ...
            const pathIdResult = (await strapi.db.queryBuilder(FOLDER_UID).max("pathId").first().execute());
            const pathId = ((pathIdResult === null || pathIdResult === void 0 ? void 0 : pathIdResult.max) || 0) + 1;
            ...
            if (!isTest)
                logger.info(`[zhao-oss] Created default folder: ${def.name} (path=${newPath})`);
        }
    }
    catch (err) {
        ...
        if (authService && typeof authService.registerPolicy === "function") {
            const createHasOssPermission = (_strapiInstance) => {
                return async (context, config) => {
                    var _a;
                    const user = ((_a = context === null || context === void 0 ? void 0 : context.state) === null || _a === void 0 ? void 0 : _a.user) || (context === null || context === void 0 ? void 0 : context.user);
                    if (!user || !user.roles || user.roles.length === 0) {
                        return { passed: false, code: "UNAUTHENTICATED", message: "未认证，请先登录" };
                    }
                    const permission = config === null || config === void 0 ? void 0 : config.permission;
                    if (!permission)
                        return { passed: true };
                    const allowedRoles = permissions_1.default[permission] || [];
            ...
            if (!isTest)
                logger.info("[zhao-oss] has-oss-permission 策略已注册到 zhao-auth");
        }
    }
    catch {
    ...
    (_c = strapi.db) === null || _c === void 0 ? void 0 : _c.lifecycles.subscribe({
        ...
        async afterCreate(event) {
            var _a;
            const siteId = (_a = event.result) === null || _a === void 0 ? void 0 : _a.id;
            if (!siteId)
                return;
            try {
                await strapi.plugin("zhao-oss").service("media-service").ensureSiteDefaultFolders(siteId);
                if (!isTest)
                    logger.info(`[zhao-oss] Created default folders for site ${siteId}`);
            }
            catch (err) {
    ...
    if (!isTest)
        logger.info("[zhao-oss] Upload lifecycle hooks registered successfully");
    ...
    if (!isTest)
        logger.info("[zhao-oss] URL rewrite middleware registered");
    // 4. 启动定时健康检查
    ...
    if (!isTest)
        logger.info("[zhao-oss] Plugin initialized successfully", {

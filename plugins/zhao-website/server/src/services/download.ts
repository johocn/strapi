...
const UID = "plugin::zhao-website.download";
...
export default ({ strapi }: { strapi: Core.Strapi }) => ({
  ...
  async findAdmin(siteId: number, query: any = {}) {
    ...
    // tagGroup 筛选：knex 查 join 表拿 download_id 列表
    if (tagGroup) {
      ...
      if (groupRow?.id) {
        ...
        if (tagIds.length > 0) {
          const downloadRows = await knex('zhao_website_downloads_tags_lnk')
            .whereIn('tag_id', tagIds).select('download_id');
          const downloadIds = [...new Set(downloadRows.map((r: any) => r.download_id))];
  ...
  async create(siteId: number, data: any) {
    const slug = data.slug || await generateUniqueSlug(strapi, UID, siteId, data.name || "untitled");
    // 真值校验（warning 级允许）
  ...
  async update(siteId: number, documentId: string, data: any) {
    ...
    if (!existing) {
      const e: any = new Error("Download not found");
      e.status = 404;
      throw e;
    }
    let updateData = { ...data };
  ...
  async incrementDownloadCount(siteId: number, documentId: string) {
    const existing = await this.findOneAdmin(siteId, documentId);
    if (!existing) return;
    await strapi.db.query(UID).update({
      where: { id: existing.id },

...
export default ({ strapi }: { strapi: Core.Strapi }) => ({
  ...
  async findAdmin(siteId: number, query: any = {}) {
    ...
    if (tagGroup) {
      ...
      if (groupRow?.id) {
        ...
        if (tagIds.length > 0) {
          const productRows = await knex('zhao_website_products_tags_lnk')
            .whereIn('tag_id', tagIds).select('product_id');
          const productIds = [...new Set(productRows.map((r: any) => r.product_id))];
  ...
  async findOneAdmin(siteId: number, documentId: string) {
    return strapi.db.query(UID).findOne({
      ...
      populate: ["coverImage", "category", "tags", "tags.tagGroup", "mainEntity", "images", "mentionedEntities",
                 "ogImage", "structuredData"],
    });
  },

...
exports.default = ({ strapi }) => {
    ...
    const validateInviteCode = async (code, appCode) => {
        ...
        try {
            ...
            // 有效期校验
            const now = new Date();
            if (inviteCode.valid_from && new Date(inviteCode.valid_from) > now)
                return null;
    ...
    const buildReferralRelation = async (params) => {
        ...
        try {
            ...
            strapi.log.info(`[sso-invite] 分销关系建立成功: invitee=${inviteeId}, inviter=${result.inviterId}, level=${result.level}, code=${inviteCode}`);

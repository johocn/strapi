...
exports.default = ({ strapi }) => {
    ...
    const login = async (params) => {
        ...
        if (type === "password") {
            if (!identifier || !password)
                throwErr("SSO_AUTH_002", 400, "identifier 和 password 必填");
        ...
        if (type === "sms") {
            if (!identifier || !code)
                throwErr("SSO_AUTH_002", 400, "identifier(mobile) 和 code 必填");
            // 校验验证码
    ...
    const verifyToken = async (token) => {
        ...
        if (payload.type !== "access")
            throwErr("SSO_AUTH_006", 401, "无效的 access token");
        ...
        if (tokenRecord === null || tokenRecord === void 0 ? void 0 : tokenRecord.revoked)
            throwErr("SSO_AUTH_007", 401, "Token 已被撤销");
        const user = await userService().findByUuid(payload.sub);
        if (!user)
            throwErr("SSO_AUTH_008", 404, "用户不存在");
        if (await userService().isBlocked(user))
            throwErr("SSO_AUTH_004", 403, "账号已被封禁");
    ...
    const refreshToken = async (refreshToken) => {
        ...
        if (!tokenRecord)
            throwErr("SSO_AUTH_010", 404, "Token 记录不存在");
        if (tokenRecord.revoked)
            throwErr("SSO_AUTH_011", 401, "Refresh token 已被撤销");
        if (new Date(tokenRecord.refresh_expires_at) < new Date())
            throwErr("SSO_AUTH_012", 401, "Refresh token 已过期");

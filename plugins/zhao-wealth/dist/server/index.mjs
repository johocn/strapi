...
class ChinawealthCollector extends BaseCollector {
  ...
  async collectByRegisterCode(registerCode) {
    ...
    try {
      ...
      await page.waitForSelector("input", { timeout: 15e3 }).catch(() => {
      ...
      const inputFound = await page.evaluate((code) => {
        ...
        for (const input of inputs) {
          ...
          for (let i = 0; i < 3 && el; i++) {
            ...
            if (text2.includes("登记编码") || text2.includes("登记代码")) {
              const nativeSetter = Object.getOwnPropertyDescriptor(
                window.HTMLInputElement.prototype,
                "value"
              )?.set;
              ...
              input.dispatchEvent(new Event("input", { bubbles: true }));
              input.dispatchEvent(new Event("change", { bubbles: true }));
              return `label:${text2.trim().substring(0, 30)}`;
            }
            el = el.parentElement;
          }
        }
      ...
      const btnClicked = await page.evaluate(() => {
        ...
        for (const btn of btns) {
          ...
          if (text2 === "查询" || text2 === "搜索" || text2 === "Search") {
            btn.click();
            return text2;
          }
        }
        for (const btn of btns) {
          const text2 = btn.textContent?.trim() || "";
          if (text2.includes("查") || text2.includes("搜")) {
            btn.click();
            return text2;
          }
        }
      ...
      if (apiResponses.length > 0) {
        ...
        for (const resp of apiResponses) {
          const product2 = this.extractProductFromApiResponse(resp.json, registerCode);
          if (product2) {
            console.log(`[chinawealth] 从 API 响应提取到产品数据: ${product2.productName}`);
            return product2;
          }
        }
   ...
   * 兼容多种响应格式
   */
  extractProductFromApiResponse(json, registerCode) {
    if (!json) return null;
    const lists = [];
   ...
   */
  async extractFromDom(page, registerCode) {
    return await page.evaluate((code) => {
      ...
      for (const el of allElements) {
        ...
        if (text.includes(code) && text.length < 1e3) {
   ...
   */
  parseTableCells(cells, fallbackCode) {
    let productName = "";
    let companyName = "";
    let prodStatus = "";
    let investNature = "";
   ...
   */
  parseApiProduct(item, fallbackCode) {
   ...
   */
  parseFromText(text, fallbackCode) {
    ...
    let status = "";
    for (const s2 of ["待售", "存续", "到期", "清算"]) {
      if (context.includes(s2)) {
        status = s2;
        break;
      }
    }

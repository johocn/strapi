/* 场地影子标签联动（对象式 lifecycle）
 * 必须是「事件 handler 对象」，DB 层执行 model.lifecycles[action](event)。
 * 事件对象不含 strapi，通过 global.strapi 取运行时实例。
 */
const UID = "plugin::zhao-point.venue";
const GROUP_SLUG = "activity-venue";

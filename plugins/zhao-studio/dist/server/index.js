...
const register = ({ strapi: strapi2 }) => {
  ...
  try {
    strapi2.plugin("zhao-common").service("i18n").setMessages({
...
const bootstrap = ({ strapi: strapi2 }) => {
  const checkPlugin = (name) => {
    try {
      ...
      if (!p) {
        strapi2.log.warn(`[zhao-studio] 依赖插件 ${name} 未启用，推广渠道跨插件功能将不可用`);
      }
    } catch {
      strapi2.log.warn(`[zhao-studio] 依赖插件 ${name} 未启用，推广渠道跨插件功能将不可用`);
    }
...
const ai = ({ strapi: strapi2 }) => ({
  ...
  async updateConfig(ctx) {
    const { data: data2 } = ctx.request.body;
    ctx.body = { data: { success: true, config: data2 } };
  },
...
const promoChannel$2 = ({ strapi: strapi2 }) => ({
  async list(ctx) {
    const service = strapi2.plugin("zhao-studio").service("promo-channel");
...
const promoCampaign$2 = ({ strapi: strapi2 }) => ({
  async list(ctx) {
    const service = strapi2.plugin("zhao-studio").service("promo-campaign");
    const { page = 1, pageSize = 10, channelId, status } = ctx.query;
    const result = await service.listCampaigns({
      page: Number(page),
      pageSize: Number(pageSize),
      channelId,
      status: status !== void 0 ? status === "true" : void 0
    });
    ctx.body = { data: result };
  },
  async findOne(ctx) {
...
const abTest$1 = ({ strapi: strapi2 }) => ({
  async list(ctx) {
    const service = strapi2.plugin("zhao-studio").service("ab-test");
  ...
  async pickVariant(ctx) {
    ...
    if (sourceTagId) {
      try {
        const tags = await strapi2.documents("plugin::zhao-track.source-tag").findMany({
        ...
        strapi2.log.warn(`[ab-test] pickVariant sourceTag lookup failed: ${err.message}`);
      }
    }
...
const channelReport$1 = ({ strapi: strapi2 }) => ({
  async getChannelReport(ctx) {
    const service = strapi2.plugin("zhao-studio").service("channel-report");
...
const collect = ({ strapi: strapi2 }) => ({
  ...
  async fetchSelectedContent(taskId, selectedTitles) {
    ...
    for (const titleUrl of selectedTitles) {
      try {
        const content = await scraper2.fetchContent(titleUrl, task.source.documentId);
...
const hasOwnInPrototypeChain = (thing, prop2) => {
  let obj = thing;
  const seen = [];
...
const getSafeProp = (obj, prop2) => obj != null && hasOwnInPrototypeChain(obj, prop2) ? obj[prop2] : void 0;
...
const isPlainObject = (val2) => {
  ...
  !hasOwnInPrototypeChain(val2, toStringTag) && !hasOwnInPrototypeChain(val2, iterator);
};
...
function merge$1(...objs) {
  ...
  const assignValue = (val2, key) => {
    ...
    if (isPlainObject(existing) && isPlainObject(val2)) {
      result[targetKey] = merge$1(existing, val2);
...
function trimSPorHTAB(str) {
  ...
  return start === 0 && end2 === str.length ? str : str.slice(start, end2);
}
...
const defaults = {
  ...
  transformResponse: [
    function transformResponse(data2) {
      ...
      if (data2 && utils$1.isString(data2) && (forcedJSONParsing && !responseType || JSONRequested)) {
...
function mergeConfig$1(config1, config2) {
  ...
  function getMergedTransitionalOption(prop2) {
    const transitional2 = utils$1.hasOwnProp(config2, "transitional") ? config2.transitional : void 0;
    if (!utils$1.isUndefined(transitional2)) {
      if (utils$1.isPlainObject(transitional2)) {
        if (utils$1.hasOwnProp(transitional2, prop2)) {
          return transitional2[prop2];
        }
      } else {
        return void 0;
      }
    ...
    if (utils$1.isPlainObject(transitional1) && utils$1.hasOwnProp(transitional1, prop2)) {
      return transitional1[prop2];
    }
    return void 0;
  }
  function mergeDirectKeys(a, b, prop2) {
    if (utils$1.hasOwnProp(config2, prop2)) {
      return getMergedValue(a, b);
    } else if (utils$1.hasOwnProp(config1, prop2)) {
      return getMergedValue(void 0, a);
    }
  }
  const mergeMap = {
  ...
  utils$1.forEach(Object.keys({ ...config1, ...config2 }), function computeConfigValue(prop2) {
    ...
    const a = utils$1.hasOwnProp(config1, prop2) ? config1[prop2] : void 0;
    const b = utils$1.hasOwnProp(config2, prop2) ? config2[prop2] : void 0;
    const configValue = merge2(a, b, prop2);
...
function resolveConfig(config2) {
  ...
  if (utils$1.isFormData(data2)) {
    if (platform.hasStandardBrowserEnv || platform.hasStandardBrowserWebWorkerEnv || utils$1.isReactNative(data2)) {
...
const xhrAdapter = isXHRAdapterSupported && function(config2) {
  return new Promise(function dispatchXhrRequest(resolve, reject) {
    ...
    if ("setRequestHeader" in request) {
      utils$1.forEach(toByteStringHeaderObject(requestHeaders), function setRequestHeader(val2, key) {
...
function estimateDataURLDecodedBytes(url) {
  ...
  for (let i = 0, len = body.length; i < len; i++) {
    ...
    if (c === 37 && isPercentEncodedByte(body, i, len)) {
      ...
      const next2 = body.charCodeAt(i + 1);
...
const factory = (env) => {
  ...
  return async (config2) => {
    ...
    try {
      ...
      const mustEnforceStreamBody = hasMaxBodyLength && (utils$1.isReadableStream(data2) || utils$1.isStream(data2));
...
function getAdapter$1(adapters2, config2) {
  ...
  for (let i = 0; i < length; i++) {
    ...
    if (adapter2 && (utils$1.isFunction(adapter2) || (adapter2 = adapter2.get(config2)))) {
      break;
    }
...
function dispatchRequest(config2) {
  ...
  const adapter2 = adapters.getAdapter(config2.adapter || defaults.adapter, config2);
  ...
  Axios$1.prototype[method] = function(url, config2) {
    return this.request(
      mergeConfig$1(config2 || {}, {
        method,
        url,
        data: config2 && utils$1.hasOwnProp(config2, "data") ? config2.data : void 0
      })
    );
  };
});
...
const CDATA$2 = ElementType.CDATA;
...
let DataNode$1 = class DataNode extends Node$1 {
  /**
...
let Text$1 = class Text extends DataNode$1 {
  constructor() {
    super(...arguments);
...
let Comment$1 = class Comment extends DataNode$1 {
  constructor() {
    super(...arguments);
...
let NodeWithChildren$1 = class NodeWithChildren extends Node$1 {
  /**
...
let Document$1 = class Document extends NodeWithChildren$1 {
  constructor() {
    super(...arguments);
...
let Element$1 = class Element extends NodeWithChildren$1 {
  /**
...
function cloneNode$1(node, recursive = false) {
  ...
  if (isText$2(node)) {
  ...
  } else if (isComment$2(node)) {
    result = new Comment$1(node.data);
  } else if (isTag$2(node)) {
    const children2 = recursive ? cloneChildren$1(node.children) : [];
    const clone2 = new Element$1(node.name, { ...node.attribs }, children2);
    ...
    result = clone2;
  } else if (isCDATA$2(node)) {
    const children2 = recursive ? cloneChildren$1(node.children) : [];
    const clone2 = new CDATA$1(children2);
    children2.forEach((child) => child.parent = clone2);
    result = clone2;
  } else if (isDocument$2(node)) {
    const children2 = recursive ? cloneChildren$1(node.children) : [];
...
function cloneChildren$1(childs) {
  const children2 = childs.map((child) => cloneNode$1(child, true));
...
class DomHandler {
  ...
  oncomment(data2) {
    ...
    const node = new Comment$1(data2);
    this.addNode(node);
    this.lastNode = node;
  }
  oncommentend() {
  ...
  onprocessinginstruction(name, data2) {
    const node = new ProcessingInstruction$1(name, data2);
    this.addNode(node);
  }
...
const attributeNames$1 = new Map([
  "definitionURL",
  "attributeName",
  "attributeType",
  "baseFrequency",
...
function formatAttributes$1(attributes2, opts) {
  ...
  if (!attributes2)
    return;
  const encode2 = ((_a2 = opts.encodeEntities) !== null && _a2 !== void 0 ? _a2 : opts.decodeEntities) === false ? replaceQuotes$1 : opts.xmlMode || opts.encodeEntities !== "utf8" ? encodeXML : escapeAttribute$1;
  return Object.keys(attributes2).map((key) => {
    var _a3, _b;
    const value = (_a3 = attributes2[key]) !== null && _a3 !== void 0 ? _a3 : "";
    if (opts.xmlMode === "foreign") {
      key = (_b = attributeNames$1.get(key)) !== null && _b !== void 0 ? _b : key;
    }
...
function renderNode$1(node, options2) {
  switch (node.type) {
    case Root:
    ...
    case Style:
    case Tag:
      return renderTag$1(node, options2);
    case Text$2:
      return renderText$1(node, options2);
  }
}
const foreignModeIntegrationPoints$1 = /* @__PURE__ */ new Set([
  "mi",
  "mo",
  "mn",
  "ms",
...
const foreignElements$1 = /* @__PURE__ */ new Set(["svg", "math"]);
function renderTag$1(elem, opts) {
  var _a2;
  if (opts.xmlMode === "foreign") {
    elem.name = (_a2 = elementNames$1.get(elem.name)) !== null && _a2 !== void 0 ? _a2 : elem.name;
...
function textContent$1(node) {
  ...
  if (hasChildren$1(node) && !isComment$2(node)) {
...
function innerText$1(node) {
  ...
  if (hasChildren$1(node) && (node.type === ElementType.Tag || isCDATA$2(node))) {
...
function getSiblings$1(elem) {
  const parent2 = getParent$1(elem);
  if (parent2 != null)
    return getChildren$1(parent2);
...
function nextElementSibling$1(elem) {
  let { next: next2 } = elem;
  while (next2 !== null && !isTag$2(next2))
    ({ next: next2 } = next2);
  return next2;
}
function prevElementSibling$1(elem) {
  let { prev: prev2 } = elem;
  while (prev2 !== null && !isTag$2(prev2))
    ({ prev: prev2 } = prev2);
  return prev2;
}
...
function compareDocumentPosition$1(nodeA, nodeB) {
  ...
  if (idx === 0) {
    return DocumentPosition$1.DISCONNECTED;
  }
  ...
  if (siblings2.indexOf(aSibling) > siblings2.indexOf(bSibling)) {
    if (sharedParent === nodeB) {
      return DocumentPosition$1.FOLLOWING | DocumentPosition$1.CONTAINED_BY;
    }
    return DocumentPosition$1.FOLLOWING;
  }
  if (sharedParent === nodeA) {
    return DocumentPosition$1.PRECEDING | DocumentPosition$1.CONTAINS;
  }
...
function uniqueSort$1(nodes) {
  ...
  nodes.sort((a, b) => {
    ...
    if (relative & DocumentPosition$1.PRECEDING) {
      return -1;
    } else if (relative & DocumentPosition$1.FOLLOWING) {
      return 1;
    }
    return 0;
  });
...
function hasClass(className) {
  return this.toArray().some((elem) => {
    const clazz = isTag$2(elem) && elem.attribs["class"];
    let idx = -1;
...
function textContent(node) {
  ...
  if (hasChildren(node) && !isComment$1(node)) {
...
function innerText(node) {
  ...
  if (hasChildren(node) && (node.type === ElementType.Tag || isCDATA$1(node))) {
...
function nextElementSibling(elem) {
  let { next: next2 } = elem;
  while (next2 !== null && !isTag$1(next2))
    ({ next: next2 } = next2);
  return next2;
}
function prevElementSibling(elem) {
  let { prev: prev2 } = elem;
  while (prev2 !== null && !isTag$1(prev2))
    ({ prev: prev2 } = prev2);
  return prev2;
}
...
function existsOne(test2, nodes) {
  return (Array.isArray(nodes) ? nodes : [nodes]).some((node) => isTag$1(node) && test2(node) || hasChildren(node) && existsOne(test2, node.children));
}
...
const Checks = {
  ...
  tag_contains(data2) {
    if (typeof data2 === "function") {
      return (elem) => isText$1(elem) && data2(elem.data);
    }
    return (elem) => isText$1(elem) && elem.data === data2;
  }
};
...
function compileRules(rules, options2, context) {
  var _a2;
  return rules.reduce((previous, rule) => previous === boolbase.falseFunc ? boolbase.falseFunc : compileGeneralSelector(previous, rule, options2, context, compileToken), (_a2 = options2.rootFunc) !== null && _a2 !== void 0 ? _a2 : boolbase.trueFunc);
}
...
function filterParsed(selector, elements, options2) {
  ...
  for (let i = 0; i < filteredSelectors.length && (found === null || found === void 0 ? void 0 : found.size) !== elements.length; i++) {
    ...
    const missing = found ? elements.filter((e) => isTag$1(e) && !found.has(e)) : elements;
    if (missing.length === 0)
      break;
...
function findFilterElements(root2, selector, options2, queryForSelector, totalLimit) {
  ...
  if (limit === 0)
    return [];
  const elemsNoLimit = sub.length === 0 && !Array.isArray(root2) ? getChildren(root2).filter(isTag$1) : sub.length === 0 ? (Array.isArray(root2) ? root2 : [root2]).filter(isTag$1) : queryForSelector || sub.some(isTraversal$1) ? findElements(root2, [sub], options2, limit) : filterElements(root2, [sub], options2);
...
function find$1(root2, query, limit = Infinity) {
  ...
  return find$2((node) => isTag$1(node) && query(node), elems, true, limit);
}
...
const parent = _singleMatcher(({ parent: parent2 }) => parent2 && !isDocument$2(parent2) ? parent2 : null, _removeDuplicates);
const parents = _matcher((elem) => {
  const matched = [];
  while (elem.parent && !isDocument$2(elem.parent)) {
    matched.push(elem.parent);
    elem = elem.parent;
  }
  return matched;
}, uniqueSort$1, (elems) => elems.reverse());
const parentsUntil = _matchUntil(({ parent: parent2 }) => parent2 && !isDocument$2(parent2) ? parent2 : null, uniqueSort$1, (elems) => elems.reverse());
function closest(selector) {
  ...
  domEach(this, (elem) => {
    if (elem && !isDocument$2(elem) && !isTag$2(elem)) {
      elem = elem.parent;
    }
    while (elem && isTag$2(elem)) {
      if (selectFn(elem, 0)) {
...
const siblings = _matcher((elem) => getSiblings$1(elem).filter((el) => isTag$2(el) && el !== elem), uniqueSort$1);
const children = _matcher((elem) => getChildren$1(elem).filter(isTag$2), _removeDuplicates);
function contents() {
...
function is(selector) {
  ...
  return typeof selector === "string" ? some(nodes.filter(isTag$2), selector, this.options) : selector ? nodes.some(getFilterFn(selector)) : false;
}
function not(match) {
...
function add(other, context) {
  ...
  const contents2 = uniqueSort$1([...this.get(), ...selection.get()]);
...
function getParse(parser) {
  return function parse2(content, options2, isDocument2, context) {
    ...
    const doc = content;
    if (!Array.isArray(doc) && isDocument$2(doc)) {
      return doc;
    }
...
function _makeDomArray(elem, clone2) {
  ...
  if ("length" in elem) {
    ...
    for (let i = 0; i < elem.length; i++) {
      ...
      if (typeof el === "object") {
        ...
        if (!("length" in el)) {
          result.push(clone2 ? cloneNode$1(el, true) : el);
          continue;
        }
      }
  ...
  return [clone2 ? cloneNode$1(elem, true) : elem];
}
...
function clone() {
  const clone2 = Array.prototype.map.call(this.get(), (el) => cloneNode$1(el, true));
...
function serializeArray() {
  return this.map((_, elem) => {
    ...
    if (isTag$2(elem) && elem.name === "form") {
...
const adapter = {
  ...
  setDocumentType(document2, name, publicId, systemId) {
    ...
    if (doctypeNode) {
      ...
      doctypeNode = new ProcessingInstruction2("!doctype", data2);
...
function parseWithParse5(content, options2, isDocument2, context) {
  var _a2;
  (_a2 = options2.treeAdapter) !== null && _a2 !== void 0 ? _a2 : options2.treeAdapter = adapter;
  if (options2.scriptingEnabled !== false) {
    options2.scriptingEnabled = true;
  }
  return isDocument2 ? parse$1(content, options2) : parseFragment(context, content, options2);
}
...
class Parser2 {
  constructor(cbs, options2 = {}) {
    ...
    this.tokenizer = new ((_d = options2.Tokenizer) !== null && _d !== void 0 ? _d : Tokenizer2)(this.options, this);
...
const parse = getParse((content, options2, isDocument2, context) => options2._useHtmlParser2 ? parseDocument(content, options2) : parseWithParse5(content, options2, isDocument2, context));
const load = getLoad(parse, (dom, options2) => options2._useHtmlParser2 ? render$2(dom, options2) : renderWithParse5(dom));
function extractTitles(html2, selector) {
  const $2 = load(html2);
  const titles = [];
  $2(selector).each((_, element) => {
...
function extractContent(html2, contentSelector, authorSelector, dateSelector) {
  const $2 = load(html2);
  const title = $2("h1").first().text().trim() || $2("title").text().trim();
  const body = $2(contentSelector).text().trim();
  const author = authorSelector ? $2(authorSelector).text().trim() : void 0;
  const date = dateSelector ? $2(dateSelector).text().trim() : void 0;
  const images = [];
  $2(contentSelector).find("img").each((_, element) => {
...
const scraper = ({ strapi: strapi2 }) => ({
  async fetchTitles(sourceId) {
    ...
    try {
      ...
      const selector = source.type === "template" && source.template ? getTemplate(source.template)?.titleSelector || source.titleSelector : source.titleSelector;
  ...
  async fetchContent(url, sourceId) {
    ...
    try {
      ...
      const template = source.type === "template" && source.template ? getTemplate(source.template) : null;
...
const aiAssist = ({ strapi: strapi2 }) => ({
  async callAI(params) {
    ...
    try {
      switch (config2.provider) {
        case "qwen":
          return await this.callQwen(params, config2, provider);
        case "wenxin":
          return await this.callWenxin(params, config2, provider);
        case "hunyuan":
          return await this.callHunyuan(params, config2, provider);
        case "spark":
          return await this.callSpark(params, config2, provider);
        case "custom":
          return await this.callCustom(params, config2);
        default:
          throw new Error("未知的AI服务提供商");
      }
    } catch (error) {
  ...
  async callQwen(params, config2, provider) {
    const response = await axios.post(
      provider.endpoint,
      {
        model: config2.model || provider.defaultModel,
        input: { prompt: params.prompt },
        parameters: {
          max_tokens: config2.maxTokens || provider.maxTokens,
          temperature: config2.temperature || provider.temperature
        }
      },
      {
        headers: {
  ...
  async callWenxin(params, config2, provider) {
    const response = await axios.post(
      `${provider.endpoint}?access_token=${config2.apiKey}`,
      {
        model: config2.model || provider.defaultModel,
  ...
  async callHunyuan(params, config2, provider) {
    const response = await axios.post(
      provider.endpoint,
      {
        Query: params.prompt,
  ...
  async callSpark(params, config2, provider) {
    const response = await axios.post(
      provider.endpoint,
      {
        header: { app_id: config2.apiKey },
        parameter: { chat: { domain: config2.model || provider.defaultModel } },
  ...
  async callCustom(params, config2) {
    const response = await axios.post(
      config2.endpoint,
      {
        prompt: params.prompt,
        max_tokens: config2.maxTokens || 2e3,
  ...
  async generateSummary(articleId, options2) {
    ...
    const summary = await this.callAI({
      prompt: `请为以下文章生成${options2?.length || 150}字的摘要：
...
${article.content}`,
  ...
  async chat(messages) {
    ...
    const conversationPrompt = conversationMessages.map((m) => `${m.role === "user" ? "用户" : "助手"}：${m.content}`).join("\n\n");
    const fullPrompt = systemPrompt ? `${systemPrompt}

${conversationPrompt}

助手：` : `${conversationPrompt}
...
function identifyPublishError(error, platform2) {
  ...
  if (error.message?.includes("platform") || error.message?.includes("平台")) {
    return { ...PublishErrors.PLATFORM_NOT_SUPPORTED, platform: platform2 };
  }
  if (error.message?.includes("401") || error.message?.includes("403") || error.message?.includes("auth")) {
    return { ...PublishErrors.AUTH_ERROR, platform: platform2 };
  }
  if (error.message?.includes("length") || error.message?.includes("长度")) {
    return { ...PublishErrors.CONTENT_TOO_LONG, platform: platform2 };
  }
  if (error.message?.includes("image") || error.message?.includes("图片")) {
    return { ...PublishErrors.IMAGE_ERROR, platform: platform2 };
  }
...
const channelAdapter = ({ strapi: strapi2 }) => ({
  ...
  async adaptContent(content, platformType) {
    const adapter2 = getPlatformAdapter(platformType);
    if (!adapter2) {
      return content;
    }
...
const internalApi = ({ strapi: strapi2 }) => ({
        ...
        if (taggedArticleIds.length > 0) {
          if (baseFilters.documentId) {
            baseFilters.documentId = {
              $in: baseFilters.documentId.$in.filter((id) => taggedArticleIds.includes(id))
            };
          } else {
  ...
  async getChannels() {
    ...
    const channels = accounts.map((account) => account.config?.channelCode).filter((channel) => channel && channel.trim() !== "");
...
const analytics = ({ strapi: strapi2 }) => ({
  ...
  async trackReadBehavior(data2) {
    const log = await strapi2.documents("plugin::zhao-studio.browser-log").create({
      data: {
        ...
        article: data2.articleId,
        sessionId: data2.sessionId,
        readDuration: data2.readDuration,
        scrollDepth: data2.scrollDepth,
        timestamp: /* @__PURE__ */ new Date()
      }
    });
    return log;
    ...
    for (const log of logs) {
      await strapi2.documents("plugin::zhao-studio.browser-log").update({
        ...
        data: {
          userId: data2.userId,
          isRegistered: true,
          registeredAt: data2.registeredAt
        }
      });
    }
  ...
  async getOverview(params) {
    ...
    const avgReadDuration = summaries.length > 0 ? summaries.reduce((sum, s) => sum + (s.avgReadDuration || 0), 0) / summaries.length : 0;
    return {
      pv: totalPv,
      uv: totalUv,
...
const aggregation = ({ strapi: strapi2 }) => ({
  async aggregateArticleDaily(date) {
    ...
    for (const [articleId, logs] of Object.entries(articleGroups)) {
      ...
      const uv = new Set(logs.map((l) => l.sessionId)).size;
      const articleReadLogs = readLogs.filter(
        (l) => (l.article?.documentId || l.article) === articleId
      );
  ...
  async aggregateAdSlotDaily(date) {
    const startDate = new Date(date);
    startDate.setHours(0, 0, 0, 0);
    const endDate = new Date(date);
    endDate.setHours(23, 59, 59, 999);
  ...
  async aggregateGlobalDaily(date) {
    const startDate = new Date(date);
    startDate.setHours(0, 0, 0, 0);
    const endDate = new Date(date);
    endDate.setHours(23, 59, 59, 999);
  ...
  async aggregateDeviceDaily(date) {
    const startDate = new Date(date);
    startDate.setHours(0, 0, 0, 0);
    const endDate = new Date(date);
    endDate.setHours(23, 59, 59, 999);
  ...
  async aggregateRegionDaily(date) {
    const startDate = new Date(date);
    startDate.setHours(0, 0, 0, 0);
    const endDate = new Date(date);
    endDate.setHours(23, 59, 59, 999);
...
const CHANNEL_UID$1 = "plugin::zhao-studio.promo-channel";
const CONFIG_UID = "plugin::zhao-studio.channel-platform-config";
const promoChannel$1 = ({ strapi: strapi2 }) => {
  ...
  return {
    async listChannels(opts) {
      const filters2 = {};
      if (opts.scene) filters2.scene = opts.scene;
      return strapi2.documents(CHANNEL_UID$1).findMany({
        filters: filters2,
    ...
    async getChannel(id) {
      const channels = await strapi2.documents(CHANNEL_UID$1).findMany({
    ...
    async createChannel(data2) {
      const existing = await strapi2.documents(CHANNEL_UID$1).findMany({
        filters: { code: data2.code }
      });
      if (existing && existing.length > 0) {
        throwErr("STUDIO_PROMO_CHANNEL_CODE_DUPLICATE", "渠道 code 重复");
      }
      return strapi2.documents(CHANNEL_UID$1).create({ data: data2 });
    },
    async updateChannel(id, data2) {
      return strapi2.documents(CHANNEL_UID$1).update({ documentId: id, data: data2 });
    },
    async deleteChannel(id) {
      return strapi2.documents(CHANNEL_UID$1).delete({ documentId: id });
    },
    async addPlatformConfig(channelId, data2) {
      const existing = await strapi2.documents(CONFIG_UID).findMany({
        filters: { channel: channelId, platform: data2.platform }
      });
      if (existing && existing.length > 0) {
        throwErr("STUDIO_PROMO_PLATFORM_CONFIG_DUPLICATE", "渠道+平台配置重复");
      }
      return strapi2.documents(CONFIG_UID).create({
        data: { channel: channelId, ...data2 }
      });
    },
    async updatePlatformConfig(configId, data2) {
      return strapi2.documents(CONFIG_UID).update({ documentId: configId, data: data2 });
    },
    async removePlatformConfig(configId) {
      return strapi2.documents(CONFIG_UID).delete({ documentId: configId });
    }
  };
};
...
const abTest = ({ strapi: strapi2 }) => {
  ...
  return {
    ...
    getExperiment,
    createExperiment: async (data2) => {
      if (!data2.channel && !data2.campaign) {
        throwErr("STUDIO_PROMO_EXPERIMENT_NOT_FOUND", "实验必须关联渠道或活动");
      }
      return strapi2.documents(EXPERIMENT_UID).create({ data: data2 });
    },
    startExperiment: async (id) => {
      ...
      return strapi2.documents(EXPERIMENT_UID).update({
        documentId: id,
        data: { status: "running", startAt: exp.startAt || (/* @__PURE__ */ new Date()).toISOString() }
      });
    },
    ...
    getExperimentReport: async (experimentId, opts) => {
      ...
      return {
        ...
        variants: (exp.variants || []).map((v) => ({
          ...
          conversionRate: clicksByVariant[v.documentId] > 0 ? Number(((ordersByVariant[v.documentId] || 0) / clicksByVariant[v.documentId] * 100).toFixed(2)) : 0
        }))
      };
    }
  };
...
const channelReport = ({ strapi: strapi2 }) => {
  const throwErr = (code, message) => {
    const err = new Error(message);
    err.code = code;
    throw err;
  ...
  const setCache = (key, data2) => {
    if (cache.size >= CACHE_MAX) {
      const firstKey = cache.keys().next().value;
      if (firstKey) cache.delete(firstKey);
    }
    cache.set(key, { data: data2, expireAt: Date.now() + CACHE_TTL_MS });
  };
  return {
    async getChannelReport(opts) {
      const cacheKey = `channel-report:${opts.channelCode}:${opts.startDate}:${opts.endDate}:${opts.groupBy || ""}`;
      const cached = getCache(cacheKey);
      if (cached) return cached;
      const channels = await strapi2.documents(CHANNEL_UID).findMany({
      ...
      const impressions = (browserLogs || []).filter((l) => l.eventType === "page-view").length;
      const adClicks = (browserLogs || []).filter((l) => l.eventType === "ad-click").length;
      const campaignIds = (channel.campaigns || []).map((c) => c.documentId);
      let couponClicks = 0;
      let orders = 0;
      let paidOrders = 0;
      ...
      const campaignCost = (channel.campaigns || []).reduce((sum, c) => sum + (Number(c.actualCost) || 0), 0);
      ...
      const report = {
        ...
        roi,
        byCampaign: (channel.campaigns || []).map((c) => ({
          campaign: c.name,
          code: c.code
        }))
      };

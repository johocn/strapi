...
async function ensureAdminUser(strapi) {
    try {
        ...
        const existing = await knex("up_users")
            ...
            .first();
        if (existing) {
            strapi.log.info(`zhao-auth: 已存在 admin 用户（id=${existing.id}, username=${existing.username}），跳过初始化`);
            return;
        }
        ...
        const dup = await knex("up_users")
            ...
            .first();
        if (dup) {
            strapi.log.warn(`zhao-auth: 用户名或邮箱已被占用（id=${dup.id}, username=${dup.username}），但该用户非 admin 角色。跳过 admin 初始化，请手动处理。`);
            return;
        }
        ...
        strapi.log.info(`zhao-auth: ✅ 已创建第一个 admin 用户（username=${username}, email=${email}）。请尽快修改默认密码。`);
    }
    catch (error) {
        strapi.log.warn(`zhao-auth: admin 用户初始化失败: ${error?.message || String(error)}`);
    }
}
/**
 * zhao-auth 引导函数
    ...
    setTimeout(async () => {
        try {
            ...
            if (results && results.length) {
                strapi.log.info(`zhao-auth: 角色初始化完成 [${results.join(", ")}]`);
            }
            // 角色初始化完成后，确保存在 admin 用户
            await ensureAdminUser(strapi);
        }
        catch (error) {
            strapi.log.warn(`zhao-auth: 角色初始化失败（可能是 content-type 尚未就绪，可通过 POST /api/zhao-auth/v1/admin/permissions/init 手动触发）: ${error?.message || String(error)}`);
        }
    }, 3000);
};

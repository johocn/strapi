...
exports.default = ({ strapi }) => ({
    async register(ctx) {
        try {
            ...
            try {
                ...
                strapi.log.warn(`[zhao-auth] 创建邀请码失败: ${e.message}`);
            }
    ...
    async adminLocal(ctx) {
        try {
            ...
            const isSuperAdmin = roles.some((r) => r === "super-admin" || r === "super_admin" || r === "SUPER_ADMIN");
            if (!isSuperAdmin) {
    ...
    async login(ctx) {
        try {
            ...
            try {
                ...
                strapi.log.warn(`[zhao-auth] 登录时创建邀请码失败: ${e.message}`);
            }
            ctx.body = {
                jwt,
                user: {
    ...
    async switchTenant(ctx) {
        try {
            ...
            const hasAccess = tenants.some((t) => t.documentId === tenantId || String(t.id) === String(tenantId));
            if (!hasAccess) {
                ctx.status = 403;

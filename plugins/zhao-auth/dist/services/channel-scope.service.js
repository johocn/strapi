...
exports.default = ({ strapi }) => ({
    ...
    async resolve(user) {
        ...
        try {
            ...
            strapi.log.warn(`[zhao-auth:channel-scope] zhao-channel 服务不可用: ${err.message}`);
            return { all: false, channelIds: [] };
        }
    },
    /**
    ...
    assertRecordInScope(scope, record, field) {
        ...
        if (Array.isArray(rel)) {
            ...
            // manyToOne: rel 是对象
            if (typeof rel.id === "number")
                recordChannelIds = [rel.id];
        }
        if (recordChannelIds.length === 0)
            return; // 无法解析 channel id，放行
    ...
    async assertChannelDocIdInScope(scope, channelDocumentId) {
        ...
        if (!channel)
            return; // channel 不存在，放行（由业务层处理 404）

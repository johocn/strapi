...
exports.ROLE_INHERITANCE = exports.ROLE_HIERARCHY = void 0;
...
function isProtected(assignment, operatorRoles) {
    if (ABSOLUTE_CORE_ROLES.includes(assignment.role))
        return true;
...
async function getRoleLevel(role) {
    if (exports.ROLE_HIERARCHY[role] != null)
...
async function computeOperatorOwnedRoles(operatorId, operatorTenantDocumentId) {
    ...
    const operatorRoles = Array.isArray(operator?.zhaoRoles)
        ? operator.zhaoRoles
    ...
    // admin 不受限：返回全部角色
    if (operatorRoles.includes("admin")) {
        const { ROLES } = await Promise.resolve().then(() => __importStar(require("../permissions")));
    ...
    try {
        ...
        const { MODULE_MANAGER_MAP } = await Promise.resolve().then(() => __importStar(require("../permissions")));
...
async function annotateUserRoles(user, _tenantDocumentId) {
    const { ROLE_LABELS } = await Promise.resolve().then(() => __importStar(require("../permissions")));
    ...
    return directRoles.map((role) => {
        const rawAssignment = rawAssignments.find((r) => typeof r === "string" ? r === role : r?.role === role);
...
exports.default = ({ strapi }) => {
    async function getUserEffectivePermissions(userId) {
        ...
        for (const role of directRoles) {
            const parents = exports.ROLE_INHERITANCE[role];
            if (parents) {
    ...
    return ({
        ...
        async assignRole(userId, role, operatorId, reason, operatorTenantDocumentId) {
            ...
            if (!isOperatorAdmin) {
                ...
                if (!ownedRoles.includes(normalizedRole)) {
                    throwErr("ROLE_006", 403, `只能分配自己拥有的角色，未拥有: ${normalizedRole}`);
                }
            }
            const operator = await strapi.db.query(USER_UID).findOne({
                where: { id: operatorId },
        ...
        async revokeRole(userId, role, operatorId, reason, operatorTenantDocumentId) {
            ...
            const assignment = assignments.find((r) => typeof r === "string" ? r === role : r?.role === role);
            if (!assignment) {
            ...
            if (!isOperatorAdmin) {
                ...
                if (!ownedRoles.includes(role)) {
                    throwErr("ROLE_006", 403, `只能撤销自己拥有的角色，未拥有: ${role}`);
                }
            }
            const newRoles = assignments.filter((r) => typeof r === "string" ? r !== role : r?.role !== role);
            await strapi.db.query(USER_UID).update({
                where: { id: userId },
                data: { zhaoRoles: newRoles },
            });
        ...
        async getAssignableRoles(operatorId, tenantDocumentId) {
            const { ROLES, ROLE_LABELS } = await Promise.resolve().then(() => __importStar(require("../permissions")));
            // 1. 取操作者拥有的角色全集
            ...
            const allRoleNames = new Set(Object.values(ROLES));
            for (const r of dbRoles) {
                if (r?.role)
                    allRoleNames.add(r.role);
            }
        ...
        computePermissions(roles) {
            ...
            for (const role of roles) {
                const rolePerms = permissions_1.DEFAULT_ROLE_PERMISSIONS[role] || [];

...
> ```bash
> git clone -b main https://github.com/johocn/strapi.git strapi
> ```
...
```bash
# 复制 PM2 配置文件到部署目录上层
cp /home/admin/strapi/docs/deployment/ecosystem.config.js /home/admin/
...
```bash
# 导出数据库（在服务器执行）
docker exec $(docker ps --filter "name=postgresql" -q) pg_dump -U postgres strapi > /home/admin/backup/strapi_$(date +%Y%m%d_%H%M%S).sql

# 恢复数据库
docker exec -i $(docker ps --filter "name=postgresql" -q) psql -U postgres strapi < /home/admin/backup/strapi_20260710_120000.sql
```
...
crontab -e
# 添加以下行：
0 3 * * * docker exec $(docker ps --filter "name=postgresql" -q) pg_dump -U postgres strapi > /home/admin/backup/strapi_$(date +\%Y\%m\%d).sql && find /home/admin/backup/ -name "strapi_*.sql" -mtime +30 -delete
```
...
# 检查 .env 数据库配置
cat /home/admin/strapi/.env | grep DATABASE
...
| 路径 | 说明 |
|---|---|
| `/home/admin/strapi` | Strapi 后端项目目录 |
| `/home/admin/strapi/.env` | 生产环境变量配置 |
| `/home/admin/ecosystem.config.js` | PM2 进程配置 |
| `/home/admin/logs` | PM2 日志目录（error.log / out.log） |
| `/home/admin/backup` | 数据库备份目录（建议创建） |
...
本部署涉及的配置文件位于项目 `docs/deployment/` 目录（本文档除外，位于项目根目录）：

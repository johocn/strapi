# 多租户网站测试数据种子脚本 Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** 创建独立种子脚本 `scripts/seed-website-test-data.js`，为多租户网站系统填充全量测试数据（3 租户 + 7 业务 CT + 5 高级 CT，共约 175 条），支持 `--clean` 清理重置。

**Architecture:** 单文件 knex 直连脚本，参考现有 `seed-tag-groups.js` 模式。内部按模块分函数，依赖顺序执行（site → category → content → advanced）。所有测试数据 document_id 固定前缀（`test` + CT 缩写），支持幂等和精确清理。

**Tech Stack:** Node.js + knex（PostgreSQL）+ dotenv

---

## 文件结构

- **Create:** `e:\code\basic\scripts\seed-website-test-data.js`（单文件，约 800-1000 行）

脚本内部结构：
```
seed-website-test-data.js
├── 常量定义         // SITES / CATEGORIES / TAGS / ARTICLES / ... 数据
├── 工具函数         // genDocId / cleanXxx / insertXxx
├── 模块 1-3         // 基础设施层（site/channel/category/tag）
├── 模块 4-7         // 业务内容层（article/product/case/faq/tutorial/compliance/download）
├── 模块 8-10        // 高级 CT（knowledge-entity/first-truth/ai-summary/lead/interaction）
├── 模块 11          // 清理逻辑（--clean）
└── main()           // 入口，解析参数 + 调度
```
...
## Task 1: 脚本骨架 + 工具函数 + 清理逻辑

**Files:**
- Create: `e:\code\basic\scripts\seed-website-test-data.js`

- [ ] **Step 1: 创建脚本骨架**

创建 `e:\code\basic\scripts\seed-website-test-data.js`，写入以下完整内容：
...
async function cleanTestData(knex) {
  ...
  // 清理各业务 CT 的 join 表（按 document_id 前缀已无法关联，需在删除前清理——已在上面步骤中处理）
  // 注意：articles_tags_lnk / cases_tags_lnk / cases_products_lnk 等需在删除主表前清理
  // 由于 knex del() 不会自动清理 join 表，需单独清理
...
Run:
```powershell
cd e:\code\basic; node scripts/seed-website-test-data.js --clean
```

Expected: 输出 `[1/12] 清理测试数据...` + 各表删除 0 条 + `[DONE] 清理完成`（空库情况下）

- [ ] **Step 3: Commit**

```powershell
cd e:\code\basic; git add scripts/seed-website-test-data.js; git commit -m "feat: 多租户测试数据种子脚本骨架 + 清理逻辑"
```

---

## Task 2: 模块 1 - site-config + channel

**Files:**
- Modify: `e:\code\basic\scripts\seed-website-test-data.js`

- [ ] **Step 1: 在 `cleanTestData` 函数之后、`main` 入口之前，添加 SITES 常量和 seedSites 函数**

在 `cleanTestData` 函数闭合 `}` 之后，插入以下代码：
...
- [ ] **Step 2: 在 main 入口的 isSeed 分支中，取消注释 seedSites 调用**

将 main 函数中的 isSeed 分支修改为：
...
- [ ] **Step 3: 验证插入 site-config + channel**

Run:
```powershell
cd e:\code\basic; node scripts/seed-website-test-data.js --clean --seed
```

Expected: 输出 3 个 site-config + 3 个 channel + 3 条关联，无报错

- [ ] **Step 4: 验证幂等性（重复执行不报错）**

Run:
```powershell
cd e:\code\basic; node scripts/seed-website-test-data.js
```

Expected: 重复执行不报唯一约束冲突

- [ ] **Step 5: Commit**

```powershell
cd e:\code\basic; git add scripts/seed-website-test-data.js; git commit -m "feat: 种子脚本 - site-config + channel 多租户基础"
```

---

## Task 3: 模块 2-3 - article-category + tag + tag-group

**Files:**
- Modify: `e:\code\basic\scripts\seed-website-test-data.js`

- [ ] **Step 1: 在 seedSites 函数之后，添加 CATEGORIES / TAG_GROUPS / TAGS 常量和 seedCategories / seedTags 函数**
...
- [ ] **Step 2: 更新 main 入口，调用 seedCategories 和 seedTags**
...
Expected: 输出 15 个 article-category + 3 个 tag-group + 15 个 tag，无报错

- [ ] **Step 4: Commit**

```powershell
cd e:\code\basic; git add scripts/seed-website-test-data.js; git commit -m "feat: 种子脚本 - article-category + tag + tag-group"
```

---

## Task 4: 模块 4-5 - article + product

**Files:**
- Modify: `e:\code\basic\scripts\seed-website-test-data.js`

- [ ] **Step 1: 在 seedTags 函数之后，添加 ARTICLES 和 PRODUCTS 常量及 seedArticles / seedProducts 函数**
...
Expected: 输出 15 篇 article + 4 个 product，无报错

- [ ] **Step 4: Commit**

```powershell
cd e:\code\basic; git add scripts/seed-website-test-data.js; git commit -m "feat: 种子脚本 - article + product 业务内容"
```

---

## Task 5: 模块 6-7 - case + faq + tutorial + compliance + download

**Files:**
- Modify: `e:\code\basic\scripts\seed-website-test-data.js`

- [ ] **Step 1: 在 seedProducts 函数之后，添加 CASES / FAQS / TUTORIALS / COMPLIANCES / DOWNLOADS 常量及对应 seed 函数**
...
Expected: 输出 case 7 + faq 15 + tutorial 9 + compliance 12 + download 6 条，无报错

- [ ] **Step 4: Commit**

```powershell
cd e:\code\basic; git add scripts/seed-website-test-data.js; git commit -m "feat: 种子脚本 - case+faq+tutorial+compliance+download"
```

---

## Task 6: 模块 8-10 - 高级 CT (knowledge-entity/first-truth/ai-summary/lead/interaction)

**Files:**
- Modify: `e:\code\basic\scripts\seed-website-test-data.js`

- [ ] **Step 1: 在 seedDownloads 函数之后，添加高级 CT 常量和 seed 函数**
...
Expected: 输出全部 16 个步骤，总计约 175 条数据，无报错
...
Expected: 各表删除测试数据，无残留

- [ ] **Step 6: Commit**

```powershell
cd e:\code\basic; git add scripts/seed-website-test-data.js; git commit -m "feat: 种子脚本 - 高级 CT (knowledge/first-truth/ai-summary/lead/interaction)"
```
...
**Files:**
- Modify: `e:\code\basic\docs\2026-07-07-zhao-website-user-guide.md`（可选，补充种子脚本说明）
...
Expected: 全部 16 步骤成功，无报错

- [ ] **Step 2: 验证数据完整性**

Run:
```powershell
cd e:\code\basic; node -e "const knex=require('knex')({client:'pg',connection:{host:process.env.DATABASE_HOST,port:process.env.DATABASE_PORT,database:process.env.DATABASE_NAME,user:process.env.DATABASE_USERNAME,password:process.env.DATABASE_PASSWORD}});(async()=>{const tables=['zhao_site_configs','zhao_channels','zhao_website_article_categories','zhao_tags','zhao_tag_groups','zhao_website_articles','zhao_website_products','zhao_website_cases','zhao_website_faqs','zhao_website_tutorials','zhao_website_compliances','zhao_website_downloads','zhao_website_knowledge_entities','zhao_website_knowledge_relations','zhao_website_first_truths','zhao_website_ai_summaries','zhao_website_leads','zhao_website_interactions'];for(const t of tables){const c=await knex(t).where('document_id','like','test%').count('* as cnt').first();console.log(t+': '+c.cnt);}await knex.destroy();})()"
```
...
zhao_site_configs: 3
zhao_channels: 3
zhao_website_article_categories: 15
zhao_tags: 15
zhao_tag_groups: 3
zhao_website_articles: 15
zhao_website_products: 4
zhao_website_cases: 7
zhao_website_faqs: 15
zhao_website_tutorials: 9
zhao_website_compliances: 12
zhao_website_downloads: 6
zhao_website_knowledge_entities: 9
zhao_website_knowledge_relations: 6
zhao_website_first_truths: 9
zhao_website_ai_summaries: 6
zhao_website_leads: 15
zhao_website_interactions: 24
```

- [ ] **Step 3: 验证 Strapi 可识别测试数据**

启动 Strapi（`npm run develop`），访问 Content Manager：
- site-config 应有 3 条测试记录（含原有默认共 4 条）
- article 应有 15 条测试记录
- 其他 CT 类似
...
Expected: 各表删除测试数据后，再次执行 Step 2 的验证脚本，所有表 count = 0

- [ ] **Step 5: Commit 最终版本**

```powershell
cd e:\code\basic; git add scripts/seed-website-test-data.js; git commit -m "test: 种子脚本验收通过 - 175 条测试数据全量验证"
```
...
### Spec coverage
- ✅ 模块 1 (site-config + channel) → Task 2
- ✅ 模块 2 (article-category) → Task 3
- ✅ 模块 3 (tag + tag-group) → Task 3
- ✅ 模块 4 (article) → Task 4
- ✅ 模块 5 (product) → Task 4
- ✅ 模块 6 (case) → Task 5
- ✅ 模块 7 (faq + tutorial + compliance + download) → Task 5
- ✅ 模块 8 (knowledge-entity + relation) → Task 6
- ✅ 模块 9 (first-truth + ai-summary) → Task 6
- ✅ 模块 10 (lead + interaction) → Task 6
- ✅ 模块 11 (清理逻辑) → Task 1
- ✅ 模块 12 (执行入口) → Task 1
- ✅ 验收标准 → Task 7
...
### Placeholder scan
- 无 TBD/TODO，所有数据完整
- 所有 join 表名已查证（zhao_website_articles_tags_lnk / zhao_website_products_tags_lnk / zhao_website_cases_tags_lnk / zhao_website_cases_products_lnk / zhao_website_tutorials_tags_lnk / zhao_website_downloads_tags_lnk）

### Type consistency
- document_id 前缀统一：testsite / testch / testcat / testtg / testtag / testart / testprod / testcase / testfaq / testtu / testcp / testdl / testke / testkr / testft / testai / testlead / testint
- siteIdMap 在所有模块间传递一致
- tagIdMap 在所有需要 tag 关联的模块间传递一致
- categoryIdMap 在所有需要 category 关联的模块间传递一致

---

**Plan complete and saved to `docs/superpowers/plans/2026-07-07-multitenant-test-data-seed.md`. Two execution options:**

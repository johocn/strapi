...
**现状细节**：
- `DEFAULT_ROLE_PERMISSIONS` 实际形状是 `Record<string, string[]>`（仅权限数组，无 displayName/isSystem/level 等元数据）
- displayName/isSystem/level 等元数据从 `ROLES` 常量和其他映射表读取
- `initDefaultRoles` 是异步函数，bootstrap.ts 通过 `setTimeout(..., 3000)` 延时 3 秒调用
- 现有逻辑：系统角色（ROLES 枚举中 27 个）每次启动强制覆盖 permissions

**改造逻辑**（保留 setTimeout 延时调用模式）：
...
**注**：现有 initDefaultRoles 使用 `strapi.db.query` 而非 `strapi.documents`，改造时保留原有 API 风格。
...
### 5.3 checkPermission 委托 + computePermissions 处理（C1 修复）

**现状**：
- `checkPermission(userId, requiredRole)` 签名实际是 `(userId: number, requiredRole: string)`，实现为 `effectiveRoles.effective.includes(requiredRole)`
- 真实调用点：`zhao-website/server/src/policies/has-website-permission.ts:12` 传入 `action`（权限 key），但测试用例传入 `"admin"`（角色名）— 现有实现语义混乱
- `computePermissions` 仍被 `controllers/role-management.ts:211` 调用，**不能删除**

**改造方案**：

1. **checkPermission 改为委托**：
```typescript
async checkPermission(userId: number, action: string, tenantDocumentId?: string) {
...
2. **保留 computePermissions**：仍用于 `controllers/role-management.ts:211` 的"批量计算多角色权限"场景（与 getMyPermissions 单用户场景不同）。但需在代码注释中标注："新代码请使用 permission.service.getMyPermissions"

3. **保留 getUserEffectivePermissions**：用于"展示用户有哪些角色"（返回角色名数组，不返回 action 数组）

4. **zhao-website 调用点同步更新**：`has-website-permission.ts:12` 现调用 `authService.checkPermission(user, action)`，需改为 `authService.checkPermission(user.id, action)`（传入 userId 而非 user 对象）

5. **测试用例更新**：`zhao-course/tests/permissions.test.ts` 等传入角色名的测试用例改为传入 action

### 5.4 路由 action 拆细说明

**现状**：zhao-studio content-api.ts 使用 `adminRoute(method, path, handler, permission)` 帮助函数，permission 参数直接传入 has-permission policy 的 `config.action` 字段（无 `plugin::` 前缀）。zhao-deal/zhao-track 的 admin-api.ts 现有 action 格式为 `zhao-<plugin>.<resource>.<action>`（如 `zhao-deal.sync.trigger`）。

**注意**：zhao-studio admin/src/index.ts 的 `app.addMenuLink` 使用 `plugin::zhao-studio.read` 格式（带前缀），与路由 has-permission 的 `zhao-studio.read`（无前缀）不一致。本 spec 不统一此不一致（超出范围），但矩阵编辑器展示的 action 列表以路由侧格式（无前缀）为准。

### 5.5 zhao-studio promo 路由 action 拆细（C6 修复）
...
### 5.6 zhao-studio 现有 11 controller 路由 action 拆细
...
### 7.0 基础搭建（zhao-auth admin 从零）

zhao-auth 当前**完全无 admin/ 目录**，需从零搭建：

```
zhao-auth/
├── admin/
│   ├── package.json          # 依赖：react, react-dom, antd, @ant-design/icons, @strapi/strapi
│   ├── tsconfig.json         # 继承 strapi 根配置
│   └── src/
│       ├── index.ts          # registerPlugin + addMenuLink
│       └── ...
└── server/（现有）
```

**UI 库选择**：使用 **antd**（与 zhao-studio 一致），不使用 Strapi Design System。原因：
- zhao-studio 现有 PluginLayout 使用 antd Layout/Menu + @ant-design/icons
- 权限组件沉淀在 zhao-auth 但主要在 zhao-studio 消费，保持 UI 库一致避免冲突
- 团队已熟悉 antd
...
| checkPermission 委托后接口签名变化 | 真实调用点 `zhao-website/has-website-permission.ts:12` 需从 `checkPermission(user, action)` 改为 `checkPermission(user.id, action)`；zhao-course 等测试用例需从传入角色名改为传入 action；Grep 确认全部调用点同步更新 |
| computePermissions 不能删除 | 仍被 `controllers/role-management.ts:211` 调用；保留但标注"新代码请使用 getMyPermissions" |
| DEFAULT_ROLE_PERMISSIONS 形状是 `Record<string, string[]>` 而非对象 | bootstrap 改造时元数据（displayName/isSystem/level）从 ROLES 常量和 ROLES_META 映射表读取，不从 DEFAULT_ROLE_PERMISSIONS 读取 |
| zhao-auth admin 从零搭建 | 需补齐 package.json/tsconfig.json；UI 库选 antd 与 zhao-studio 一致 |

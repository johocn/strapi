...
**委托关系处理**：工厂内部第 1013-1014 行有一个 `getUserLevel` 方法委托给模块级 `getUserLevel` 函数：

```typescript
// 现有（模块级函数 + 工厂方法委托）
async function getUserLevel(userId) { ... }  // 模块级，引用全局 strapi
export default ({ strapi }) => ({
  async getUserLevel(userId) { return getUserLevel(userId); }  // 委托
});

// 修复后（函数移入闭包，方法委托引用闭包内函数）
export default ({ strapi }) => {
  async function getUserLevel(userId) { ... }  // 闭包内，引用闭包 strapi
  return {
    async getUserLevel(userId) { return getUserLevel(userId); }  // 委托闭包内函数
  };
};
```

方法体内的 `getUserLevel(userId)` 会引用闭包作用域的函数声明（JS 作用域规则），不会递归调用方法自身（方法自身需 `this.getUserLevel()`）。4 个函数同名移入闭包均安全。

**外部调用验证**：`controllers/permission.ts:92` 通过 `strapi.plugin('zhao-auth').service('role-management').getUserLevel(operatorId)` 调用，走的是工厂方法（第 1013 行），不是直接 import 模块级函数。移入闭包后工厂方法仍存在，外部调用不受影响。
...
zhao-studio 现有 jest.config.ts 不支持 tsx 组件测试：
- `testMatch` 只有 `**/*.test.ts`（不含 .test.tsx）
- `moduleFileExtensions` 只有 `['ts', 'js', 'json']`（不含 tsx）
- `testEnvironment` 是 `node`（不是 jsdom）
- `package.json` devDependencies 缺少 `@testing-library/react`、`@testing-library/jest-dom`、`jest-environment-jsdom`

**配置改动**：

1. `package.json` devDependencies 新增：
   - `@testing-library/react`
   - `@testing-library/jest-dom`
   - `jest-environment-jsdom`

2. `tests/jest.config.ts` 修改：
   - `testMatch` 改为 `['**/*.test.ts', '**/*.test.tsx']`
   - `moduleFileExtensions` 改为 `['ts', 'tsx', 'js', 'json']`

3. 测试文件顶部加 `@jest-environment jsdom` 注释（逐文件指定，不全局改 testEnvironment，避免影响现有 node 测试）
...
| 2 | 修改 | `zhao-studio/package.json` | devDependencies 加测试依赖 |
| 2 | 修改 | `zhao-studio/tests/jest.config.ts` | testMatch + moduleFileExtensions 加 tsx |
...
| role-management 函数移入闭包后委托关系断裂 | 低（JS 作用域规则保证方法体内引用闭包函数声明） | 移入后运行全量测试验证，特别检查 getUserLevel 委托 |
| zhao-studio 安装新测试依赖可能引入版本冲突 | 中 | 安装时用与 zhao-auth 相同版本，安装后运行全量测试 |
| tsx 测试配置可能影响现有 node 测试 | 低（逐文件指定 jsdom，不全局改 testEnvironment） | 配置后运行全量测试确认现有测试不受影响 |

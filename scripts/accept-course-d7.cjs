/* 课程 D7 转化归因验收
 * 用法: cd e:\code\basic && node scripts/accept-course-d7.cjs
 * 依赖: 本地 dev 1337 运行中(127.0.0.1:1337)。
 * 覆盖:
 *  a) 默认查询(近30天): course.d7 触达 job → 桥接 up_user → 窗口内 course-enrollment 转化统计
 *  b) 窗口边界: 窗口外 enrollment 不计
 *  c) from>to -> 400
 *  d) 清理零残留(job/enrollment/rule/template/version/user + 各 join lnk)
 * schema 要点(已对真实库核对):
 *  - sso_msg_jobs 无 user 列，关系经 join 表 sso_msg_jobs_user_lnk
 *  - course-enrollment 表 zhao_course_enrollments, enrolled_at 列; user 关系经
 *    zhao_course_enrollments_user_lnk(course_enrollment_id,user_id)
...
async function cleanup() {
  // enrollments + 其 user join lnk(按 qr_/qd_ up_users 定位)
  ...
  // up_users(先清 sso 与 job 依赖,再删 up_users)
  ...
  // jobs + lnks(按 qr_/qd_ 前缀 dedupe_key 定位)
...
(async () => {
  ...
  const tpl = await qa(
    ...
    [crypto.randomUUID(), 'qd_tpl_' + ts, 'QD模板', 'wechat']
  );
  ...
  const ver = await qa(
    ...
    [crypto.randomUUID(), 'qd_ver_' + ts, 'QD版本']
  );
  ...
  // 1.2 sop-rule: scene=course.d7, conversion_window_days=7
  const rule = await qa(
    ...
    [crypto.randomUUID(), 'qd_rule_' + ts, 'QD课后D7规则', 'cron', 'course.d7', 'qd_tpl_' + ts, 7]
  );
  ok('建 course.d7 sop-rule(conversion_window_days=7)', !!rule[0].id, `ruleId=${rule[0].id}`);
  ...
  const upA = await qa(
    ...
    [crypto.randomUUID(), 'qd_A_' + ts, 'qd_a_' + ts + '@qd.vip', hash, 'local']
  );
  ...
  const ssoA = await qa(
    ...
    [crypto.randomUUID(), crypto.randomUUID(), 'qd_A_' + ts, 'qd_a_' + ts + '@qd.vip', hash, 'active', 'accept']
  );
  const ssoAId = ssoA[0].id;
  ok('建用户A(sso+up 按 username 桥接)', !!upAId && !!ssoAId, `ssoA=${ssoAId} upA=${upAId}`);

  // 1.4 course.d7 触达 job A: scene=course.d7, sent_at=1天前, user 经 join 表
  const jobA = await qa(
    "INSERT INTO sso_msg_jobs (document_id,scene,provider,status,retry_count,dedupe_key,created_at,updated_at) VALUES ($1,'course.d7', $2,'sent',0,$3,now(),now()) RETURNING id",
    [crypto.randomUUID(), 'wechat', 'qd_ded_A_' + ts]
  );
  ...
  ok('造 course.d7 触达 job A(sent, 1天前送达)', !!jobAId, `jobA=${jobAId}`);

  // 1.5 enrollment A: user→upA, status=enrolled, enrolled_at=now()(窗口内)
  const enrA = await qa(
  ...
  const enrAId = enrA[0].id;
  await qa('UPDATE zhao_course_enrollments SET enrolled_at = now() WHERE id=$1', [enrAId]);
  await qa('INSERT INTO zhao_course_enrollments_user_lnk (course_enrollment_id,user_id) VALUES ($1,$2)', [enrAId, upAId]);
  ok('造 enrollment A(enrolled, now() 窗口内)', !!enrAId, `enrA=${enrAId}`);

  // ---------- 2. 默认查询(近30天)断言 ----------
  let g = await req('GET', '/api/zhao-sso/v1/admin/msg/course-d7-stats', null, token);
  let d = g.data && g.data.data;
  ok('默认查询 200 且有 data', g.status === 200 && !!d, `status=${g.status}`);
  if (!d || g.status !== 200) { console.error('course-d7-stats 异常:', JSON.stringify(g.data)); }
  ...
  // ---------- 3. 窗口边界: 用户B 8天前送达的 job, enrollment_at=now()(=sent_at+8天, 窗口外) ----------
  const upB = await qa(
    'INSERT INTO up_users (document_id,username,email,password,provider,confirmed,blocked,created_at,updated_at) VALUES ($1,$2,$3,$4,$5,true,false,now(),now()) RETURNING id',
    [crypto.randomUUID(), 'qd_B_' + ts, 'qd_b_' + ts + '@qd.vip', hash, 'local']
  );
  const upBId = upB[0].id;
  const ssoB = await qa(
    'INSERT INTO sso_users (document_id,uuid,username,email,password_hash,status,register_channel,login_count,created_at,updated_at) VALUES ($1,$2,$3,$4,$5,$6,$7,0,now(),now()) RETURNING id',
    [crypto.randomUUID(), crypto.randomUUID(), 'qd_B_' + ts, 'qd_b_' + ts + '@qd.vip', hash, 'active', 'accept']
  );
  const ssoBId = ssoB[0].id;
  const jobB = await qa(
    "INSERT INTO sso_msg_jobs (document_id,scene,provider,status,retry_count,dedupe_key,created_at,updated_at) VALUES ($1,'course.d7', $2,'sent',0,$3,now(),now()) RETURNING id",
    [crypto.randomUUID(), 'wechat', 'qd_ded_B_' + ts]
  );
  ...
  const enrBId = enrB[0].id;
  await qa('UPDATE zhao_course_enrollments SET enrolled_at = now() WHERE id=$1', [enrBId]); // = sent_at+8天
  await qa('INSERT INTO zhao_course_enrollments_user_lnk (course_enrollment_id,user_id) VALUES ($1,$2)', [enrBId, upBId]);
  ok('造用户B+job B(8天前送达)+enrollment B(窗口外)', !!jobBId && !!enrBId, `jobB=${jobBId} enrB=${enrBId}`);

  g = await req('GET', '/api/zhao-sso/v1/admin/msg/course-d7-stats', null, token);
  ...
  // ---------- 4. from>to -> 400 ----------
  g = await req('GET', '/api/zhao-sso/v1/admin/msg/course-d7-stats?from=' + encodeURIComponent('2026-09-01') + '&to=' + encodeURIComponent('2026-01-01'), null, token);
  ...
  const rc = (await qa(`SELECT
      ...
      (SELECT count(*)::int FROM sso_msg_jobs_user_lnk WHERE msg_job_id IN (SELECT id FROM sso_msg_jobs WHERE dedupe_key LIKE $8)) ju`,
    [PREFIX + '%', PREFIX + '%', PREFIX + '%', PREFIX + '%', PREFIX + '%', PREFIX + '%', PREFIX + '%', PREFIX + '%']))[0];
  ok('清理零残留(job/rule/template/version/sso_user/up_user/enrollment/job-user join 均 0)',
    rc.j === 0 && rc.r === 0 && rc.t === 0 && rc.v === 0 && rc.u === 0 && rc.uu === 0 && rc.e === 0 && rc.ju === 0,
    `j=${rc.j} r=${rc.r} t=${rc.t} v=${rc.v} u=${rc.u} uu=${rc.uu} e=${rc.e} ju=${rc.ju}`);

  console.log(`\n=== 课程 D7 转化归因验收: PASS=${PASS} FAIL=${FAIL} ===`);
